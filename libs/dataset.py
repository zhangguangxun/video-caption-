        ...
        with open(os.path.join(dataset_dir, ann_file), "r") as f:
            ...
            for sentence in ann["sentences"]:
                ...
                if video_id not in self.video_ids:
                    ...
                    self.data[
                        self.__len__() - 1]["cap_id"].append(sentence["sen_id"]) # 默认ann["sentences"]的标注是有序的。
    """
    obj包含video的标识符号"video_id"，video的读取路径"video_path"，所包含的文本内容"caption"和文本所对应得id："cap_id"。
    """
    ...
    def __getitem__(self, index):
        ...
        if self.align_size:
            ...
            if ft.size() != self.align_size:
                ft = F.interpolate(
                    ft, size=self.align_size, mode='trilinear', align_corners=False).squeeze(0)
        """
        # 将ft.unsqueeze(0)更改为ft,即其本身的输入维度为[1,1024,m,7,7]不需要额外的扩展第一维度。其输出为统一的维度:[1024,10,7,7]
        """
